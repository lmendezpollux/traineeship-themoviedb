<template>
  <section class="pt-20 grid h-screen justify-center align-middle items-center">
    <div class="grid justify-center">
      ...
      <div>
        ...
        <label
          :class="[estado ? 'text-white bg-transparent' : 'text-greyMain']"
          class="block font-bold mb-2 mt-2"
          for=""
          >Contraseña</label
        >
        <input
...
export default {
  ...
  methods: {
    ...
    logWithEmail() {
      try {
        if (this.email.length === 0 || this.contraseña.length === 0) {
          alert("Rellena los campos");
        } else {
          const auth = getAuth();
          signInWithEmailAndPassword(auth, this.email, this.contraseña).then(
            (userCredential) => {
